...
const importData = async (req, res) => {
  try {
    ...
    const { clients = [], projects = [], invoices = [], payments = [] } = req.body;
    ...
    return res.status(200).json({ success: true, imported: importStats });
  } catch (error) {
    console.error("Migration import error:", error);
    return res.status(500).json({ success: false, message: "Internal server error processing migration data" });
  }
};
