...
export function GlobalRecordPaymentDialog({ open, onOpenChange, onSuccess }) {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      const activeInvoices = (res.data.invoices || []).filter(inv => 
  ...
  useEffect(() => {
    if (selectedInvoice) {
      setFormData(prev => ({ ...prev, amount: remainingBalance > 0 ? remainingBalance : 0 }));
    }
  }, [selectedInvoiceId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await API.post(`/invoices/${selectedInvoiceId}/payments`, formData);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select an invoice to record a new payment.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          <div className="flex flex-col gap-3">
            ...
            <Select value={selectedInvoiceId} onValueChange={setSelectedInvoiceId} disabled={isLoadingInvoices}>
                <SelectTrigger>
                    <SelectValue placeholder={isLoadingInvoices ? "Loading..." : "Select an invoice"} />
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button type="submit" disabled={isSubmitting || !selectedInvoiceId || remainingBalance <= 0}>
